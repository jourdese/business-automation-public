...
const catalog = JSON.parse(readFileSync(new URL("../data/marinara.json", import.meta.url), "utf8"));
export function seedSql(asOf = "2026-09-19") {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(asOf)) throw new Error("Use an explicit YYYY-MM-DD seed date");
  return `-- Generated from data/marinara.json. Additive seed; never resets an existing restaurant.
do $seed$
declare data jsonb:='${JSON.stringify(catalog).replaceAll("'", "''")}'::jsonb; as_of date:='${asOf}'; b uuid; r uuid; sid uuid; iid uuid; mid uuid; oid uuid; pid uuid;
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  writeFileSync("supabase/seed.sql", seedSql(asOf));
  console.log(`Prepared deterministic Marinara seed for ${asOf}. No database connection was made.`);
}
