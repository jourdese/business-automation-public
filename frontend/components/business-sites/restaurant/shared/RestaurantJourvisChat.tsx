...
import {
  createBusinessDemoBootstrap,
  isExpectedBusiness,
} from "@/lib/jourvis/business-demo-bootstrap";
...
export default function RestaurantJourvisChat({
  business,
  presentation,
  className = "",
}: {
  business: InitialBusiness;
  presentation: RestaurantChatPresentation;
  className?: string;
}) {
      ...
      createBusinessDemoBootstrap(
        {
          ...
          adapterKey: business.adapterKey,
          presetKey: business.presetKey,
        },
        {
  ...
  const accept = useCallback(
    async (result: DemoReply, visible = true) => {
      if (visible) {
        if (result.reply) {
          setMessages((current) => {
            const id =
              result.receiptId || result.messageId || crypto.randomUUID();
            if (
              current.some(
                (message) => message.id === id && message.role === "jourvis",
              )
            )
              return current;
            return [
              ...current,
              { id, role: "jourvis" as const, text: result.reply! },
            ].slice(-40);
          });
        }
        setReply(result);
      ...
      if (result.requiresAcknowledgement) {
        await demoRequest(
          "ack",
          { receiptId: result.receiptId, messageId: result.messageId },
          scope,
        );
      }
    },
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Jourvis is temporarily unavailable.",
        );
        return false;
      } finally {
  ...
  const sendMessage = useCallback(
    async (text: string, choiceId?: string, retry?: PendingMessage) => {
      const clean = text.trim();
      if (!clean || busyRef.current) return;
      if (!readyRef.current && !(await ensureRestaurant())) return;
      if (busyRef.current) return;
      const request: PendingMessage = retry || {
      ...
      if (!retry)
        setMessages((current) =>
          [
            ...current,
            { id: request.id, role: "customer" as const, text: clean },
          ].slice(-40),
        );
      setDraft("");
      busyRef.current = true;
      ...
      try {
        const result = await demoRequest(
          "turn",
          {
            ...
            text: request.text,
            ...(request.choiceId ? { choiceId: request.choiceId } : {}),
          },
          scope,
        );
        ...
        if (!isExpectedBusiness(result, business)) {
          ...
          setReady(false);
          setError(
            `This session left ${business.displayName} demo. Reconnect to continue with this restaurant.`,
          );
        }
      } catch (err) {
        setError(
          err instanceof Error
            ? err.message
            : "The connection paused. Retry the message to check its result.",
        );
      } finally {
        busyRef.current = false;
        setBusy(false);
  ...
  useEffect(() => {
    window.dispatchEvent(
      new CustomEvent(presentation.stateEvent, { detail: { open } }),
    );
    if (!open) return;
  ...
  const panel = open ? (
    <aside
      id={presentation.panelId}
      className={`rib-jourvis-chat ${styles.theme} ${styles.panel}${className ? ` ${className}` : ""}`}
      role="dialog"
      aria-label="Chat with Jourvis"
      aria-describedby={presentation.disclaimerId}
      onKeyDown={(event) => {
        if (event.key === "Escape") {
    ...
    >
      <header className="rib-jourvis-chat-head">
        <div className="rib-jourvis-chat-identity">
          <span className="rib-jourvis-mini">
            <JourvisCompanion size={34} molecules={false} />
          </span>
          <div>
            <strong>Jourvis × {business.displayName}</strong>
            <span>
              {ready
                ? `${business.displayName} restaurant preset loaded`
                : busy
                  ? "Connecting to the restaurant demo…"
                  : "Restaurant demo not connected"}
            </span>
          </div>
        </div>
      ...
      <div className="rib-jourvis-chat-intro">
        ...
        <span>
          Explore menu questions, meal plans and reservation enquiries. Confirm
          current prices with the restaurant.
        </span>
      </div>
      <div
        className="rib-jourvis-log"
        ref={logRef}
        role="log"
        aria-live="polite"
      ...
      >
        {!messages.length && !busy ? (
          <p className="rib-chat-empty">
            Your conversation will appear here once Jourvis connects.
          </p>
        ) : null}
        {messages.map((message) => (
          <div
            key={`${message.role}:${message.id}`}
            className={`rib-chat-message ${message.role}`}
          >
            {message.role === "jourvis" ? (
              <span className="rib-chat-avatar" aria-hidden>
                <JourvisCompanion size={28} molecules={false} />
              </span>
            ) : null}
            <p>{message.text}</p>
          </div>
        ...
        {busy ? (
          <div
            className="rib-chat-typing"
            aria-label={ready ? "Jourvis is replying" : "Connecting to Jourvis"}
          >
            <span />
            <span />
            <span />
          ...
          {reply?.choices?.length
            ? reply.choices.slice(0, 4).map((choice) => (
                <button
                  ...
                  onClick={() => void sendMessage(choice.title, choice.id)}
                >
                  {choice.title}
                </button>
              ))
            : presentation.quickPrompts.map((prompt) => (
                <button
                  key={prompt}
                  type="button"
                  disabled={busy || !!pending}
      ...
      {error ? (
        <p className="rib-chat-error" role="alert">
          {error}
        </p>
      ) : null}
      {error && !busy ? (
        <button
          className="rib-chat-retry"
          type="button"
          onClick={() => {
            if (pending)
              void sendMessage(pending.text, pending.choiceId, pending);
            else void ensureRestaurant();
          }}
        >
          {pending ? "Retry the same message" : "Retry connection"}
        </button>
      ) : null}
      <form
      ...
      >
        <label className="sr-only" htmlFor={presentation.messageId}>
          Message Jourvis
        </label>
        <textarea
        ...
        <button
          type="submit"
          disabled={busy || !ready || !draft.trim() || !!pending}
          aria-label="Send message"
        >
          <Send size={18} aria-hidden />
        </button>
      </form>
      <p id={presentation.disclaimerId} className="rib-chat-disclaimer">
        Jourvis demo · no real kitchen order or table reservation is sent.
      </p>
    </aside>
  ) : null;
  ...
  return (
    <>
      <JourvisLauncher
        ...
        controls={presentation.panelId}
        label={`Chat with Jourvis for ${business.displayName}`}
