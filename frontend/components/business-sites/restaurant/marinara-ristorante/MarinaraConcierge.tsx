...
export default function MarinaraConcierge({
  business,
  draft,
  onDraft,
  inputRef,
  ...
  async function copyDraft() {
    ...
    try {
      ...
      if (id === copyId.current)
        setStatus(
          "Copy is unavailable here. Select the text above and copy it manually.",
        );
    }
  }
  return (
    <section
      ...
      id="marinara-jourvis"
      aria-labelledby="marinara-jourvis-title"
    >
      <div className={styles.conciergeHead}>
        <span>
          <JourvisCompanion size={36} molecules={false} />
        </span>
        <div>
          <h3 id="marinara-jourvis-title">Jourvis</h3>
          <p>Dining companion</p>
        </div>
        <b>{config.runtimeEnabled ? "Demo" : "Preview"}</b>
      </div>
      <p className={styles.conciergeLead}>
        Need a second opinion on the table?
      </p>
      <p className={styles.panelNote}>
        {config.runtimeEnabled
          ? "Ask about menu choices and prepare an enquiry."
          : config.runtimeNotice}
      </p>
      <div className={styles.quickPrompts}>
        {config.chat.quickPrompts.map((prompt) => (
          <button
            key={prompt}
            type="button"
            onClick={() => {
      ...
      <div className={styles.conciergeActions}>
        ...
        >
          <Copy size={15} aria-hidden /> Copy enquiry
        </button>
        {config.runtimeEnabled ? (
          <button
            ...
            onClick={() =>
              window.dispatchEvent(
                new CustomEvent(config.chat.openEvent, {
                  detail: { prompt: draft },
                }),
              )
            }
          >
            <MessageCircle size={15} aria-hidden /> Open chat
          </button>
        ) : null}
      </div>
      <p className={styles.copyStatus} role="status">
        {status || "No order, booking or message is submitted."}
      </p>
      {config.runtimeEnabled ? (
        <RestaurantJourvisChat business={business} presentation={config.chat} />
      ) : null}
    </section>
  );
}
