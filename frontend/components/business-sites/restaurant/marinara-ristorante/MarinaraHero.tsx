...
export default function MarinaraHero({ onAsk }: { onAsk: () => void }) {
  const pasta =
    foodGallery.find((item) => item.id === "alfredo") ?? foodGallery[1];
  const pizza =
    foodGallery.find((item) => item.id === "burrata") ?? foodGallery[7];
  return (
    <section
      className={styles.hero}
      id="marinara-top"
      aria-labelledby="marinara-title"
    >
      <div className={styles.heroCopy}>
        ...
        <div className={styles.heroNote}>
          <span>Archived menu references · demo planning only</span>
      ...
      <div
        ...
        aria-label="Marinara food and restaurant imagery"
      >
        <figure className={styles.heroMain}>
          <Photo
            src={assets.interior}
            alt="Marinara dining room"
            loading="eager"
          />
