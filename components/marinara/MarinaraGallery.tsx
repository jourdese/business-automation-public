"use client";
import { foodGallery, type MarinaraGalleryImage } from "./assets";
  ...
  return (
    ...
    >
      <div className={styles.galleryHeading}>
        ...
        <p>
          Every supplied Marinara image is part of this concept—pasta, pizza, sweets, drinks, shared
          sets and the room itself.
        </p>
      </div>
      <div className={styles.galleryRail} aria-label="Marinara food gallery">
        {foodGallery.map((image, index) => (
          <figure key={image.id} className={index % 5 === 0 ? styles.galleryLarge : ""}>
            <button type="button" onClick={() => onView(image)} aria-label={`View ${image.label}`}>
